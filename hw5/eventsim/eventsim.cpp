...
int main(int argc, char* argv[])
{
    ...
    while (!pq.empty())
    {
      Event* currentEvent = pq.top();
      EventList consequences = currentEvent->process();
      std::cout << consequences.size() << std::endl;
      pq.pop(); //deallocates event via internal vector pop_back call
      for (int i = 0; i < consequences.size(); i++)
        pq.push(consequences[i]);
    }

    ofstream outfile(argv[2]);
    streets.outputCompletedVehicles(outfile);
