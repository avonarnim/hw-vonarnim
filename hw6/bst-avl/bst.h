...
protected:
    ...
    int subTreeHeight(Node<Key, Value>* current, int depth) const;
...
BinarySearchTree<Key, Value>::iterator::operator!=(
    const BinarySearchTree<Key, Value>::iterator& rhs) const
{
    return current_ != rhs.current_;  //or just do *this != *rhs;
}
...
BinarySearchTree<Key, Value>::iterator::operator++()
{ //re-evaluate the logic please
    current_ = successor(current_); //this is a pre-increment
    return *this;
}
...
{
    BinarySearchTree<Key, Value>::iterator start(getSmallestNode());
    return start;
}
...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    if (duplicate != NULL)
    {
      duplicate->getValue() = keyValuePair.second;
      return;
    }
    ...
    else if (keyValuePair.first > insertion->getParent()->getKey())
      {
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    if (rmNode->getLeft() == NULL && rmNode->getRight() == NULL)
    {
      ...
      if (rmParent != NULL){
        if (rmNode->getParent()->getRight() == rmNode)
        {
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
  ...
  if (current->getLeft() != NULL)  //if the node has a left subtree
  {
  ...
  else  //no left subtree. need to find a parent to whom this node is a right descendent
  {
    while (temp->getParent() != NULL && temp->getRight() != current)
    {
      current = temp;
      temp = current->getParent();
    }
    if (temp->getParent() == NULL)
      return NULL;   //root reached, and no predecessor exists
    else
      return temp;
  }
}
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::clearHelper(Node<Key, Value>* passing)
{
  if (passing == NULL)
    return;
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
    ...
    {
      if (searching->getKey() == key)
        return searching;
      else if (searching->getKey() > key)
        searching = searching->getLeft();
      else searching = searching->getRight();
    }
    return NULL;
}
...
bool BinarySearchTree<Key, Value>::areSubsBalanced(Node<Key, Value>* head) const
{
  if (head == NULL)
    return true;
  int difference = subTreeHeight(head->getLeft(), 1) - subTreeHeight(head->getRight(), 1);
  if (difference <= 1 && difference >= -1)
    return areSubsBalanced(head->getLeft()) && areSubsBalanced(head->getRight());
  else
    return false;
}
...
int subTreeHeight(Node<Key, Value> * current, int depth) const
{
    ...
    return std::max(getSubtreeHeight(current->getLeft(), depth + 1),
                    getSubtreeHeight(current->getRight(), depth + 1)) + 1;
}
